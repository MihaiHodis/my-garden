...
Base URL: `http://localhost:5000/` (adjust for local dev)
...
- POST `/api/sensors/data` (x-api-key)
  - Body example: `{ "device_uid": "ESP32_01", "temp": 23.5, "humidity": 60, "soil_moisture": 250 }`
- GET `/api/actuators/status` (x-api-key)
...
curl -H "x-api-key: <DEVICE_API_KEY>" \
  http://localhost:5000/api/actuators/status?device_uid=ESP32_01
```
